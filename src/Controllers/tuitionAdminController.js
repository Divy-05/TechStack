...
//     try {
//         const { name, email, password, contact_number, institution_name, address } = req.body;

//         if (!name || !email || !password || !contact_number || !institution_name || !address) {
...
//             return res.status(400).json({ message: "Invalid contact number format. Use +91XXXXXXXXXX" });
//         }

//         const adminExists = await TuitionAdmin.findOne({ $or: [{ email }, { contact_number }] });
//         if (adminExists) {
//             return res.status(409).json({ message: "Admin with this email or contact number already exists" });
//         }
...
const registerTuitionAdmin = async (req, res) => {
    try {
        const { name, email, password, contact_number, institution_name } = req.body;
...
const loginTuitionAdmin = async (req, res) => {
    try {
        ...
        const tuitionAdmin = await TuitionAdmin.findOne({ $or: [{ email }, { contact_number }] });
        if (!tuitionAdmin) {
...
const updateTuitionAdminProfile = async (req, res) => {
    try {
        ...
        tuitionAdmin.institution_name = institution_name || tuitionAdmin.institution_name;
        ...
        tuitionAdmin.updatedAt = new Date().toLocaleString("en-US", { timeZone: "Asia/Kolkata" });
